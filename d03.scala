import scala.annotation.tailrec
  ...
  @tailrec
  private def up(i: Int, ps: List[Point]): List[Point] =
    if (i <= 0) ps else up(i - 1, ps.head.up +: ps)

  @tailrec
  private def down(i: Int, ps: List[Point]): List[Point] =
    if (i <= 0) ps else down(i - 1, ps.head.down +: ps)

  @tailrec
  private def left(i: Int, ps: List[Point]): List[Point] =
    if (i <= 0) ps else left(i - 1, ps.head.left +: ps)

  @tailrec
  private def right(i: Int, ps: List[Point]): List[Point] =
    if (i <= 0) ps else right(i - 1, ps.head.right +: ps)

  private def wirePath(xs: String): List[Point] = {
    xs.split(",").foldLeft(Point(0,0) :: Nil) {
      case (ps, s) if s.startsWith("U") => up(s.tail.toInt, ps)
      case (ps, s) if s.startsWith("D") => down(s.tail.toInt, ps)
      case (ps, s) if s.startsWith("R") => right(s.tail.toInt, ps)
      case (ps, s) if s.startsWith("L") => left(s.tail.toInt, ps)
    }.reverse
  }

  private def closest(l: List[Point], r: List[Point]): Option[Int] = {
    l.intersect(r).filterNot(_ == Point(0, 0)).map(p => math.abs(p.x) + math.abs(p.y)).sorted.headOption
  }

  private def shortest(l: List[Point], r: List[Point]): Option[Int] = {
    val candidates = l.intersect(r).filterNot(_ == Point(0, 0))
    val leftDistances = candidates.map(candidate => candidate -> l.indexOf(candidate))
      .filter(_._2 >= 0).toMap
    candidates.map(candidate => candidate -> r.indexOf(candidate))
      .filter(_._2 >= 0)
      .toMap
      .filter{ case (k, _) => leftDistances.contains(k) }
      .map { case (k, v) => k -> (leftDistances(k), v) }
        .values
      .toList
        .sortBy(p => p._1 + p._2)
      .headOption
